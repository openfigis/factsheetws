 ...
 * 
 * In order to have a testcase where a factsheet has a representation in 2
 * languages and where from these 2 the languages should be added to the title
 * list.
 * 
 * 
...
public class FisheryResourcesServiceIsolationTest {
	...
	public void testRetrieveStocksBySpeciesResponseIsolated() {
		...
		// languages
		assertFalse(response.getAqResList().get(0).getTitleList().get(0).getContent().equals(
				response.getAqResList().get(0).getTitleList().get(1).getContent()));
		// the both urls should not be the same because the language is
		// different.
		assertFalse(response.getAqResList().get(0).getIdentifierList().get(0).getContent().equals(
				response.getAqResList().get(0).getIdentifierList().get(1).getContent()));
